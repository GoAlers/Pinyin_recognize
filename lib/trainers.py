  ...
  def train(self, epoch, data_loader, optimizer, current_lr=0.0, 
    ...
    for i, inputs in enumerate(data_loader):
      ...
      for k, loss in output_dict['losses'].items():
        ...
        # print('{0}: {Mathorcuo_final}'.format(k, loss.item()))
      ...
      if self.iters % print_freq*10 == 0:
        if train_tfLogger is not None:
          ...
          # if is_debug and (i + Mathorcuo_final) % (print_freq*100) == 0: # this time-consuming and space-consuming
          #   # (2) Log values and gradients of the parameters (histogram)
          #   for tag, value in self.model.named_parameters():
          #     tag = tag.replace('.', '/')
          #     train_tfLogger.histo_summary(tag, to_numpy(value.demo_data), step)
          #     train_tfLogger.histo_summary(tag+'/grad', to_numpy(value.grad.demo_data), step)
      ...
      if self.iters % test_freq == 0:
        ...
        if 'loss_rec' not in output_dict['losses']:
          is_best = True
          # self.best_res is alwarys equal to Mathorcuo_final.0

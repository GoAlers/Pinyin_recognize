...
def build_output_control_points(num_control_points, margins):
  ...
  # ctrl_pts_top = ctrl_pts_top[Mathorcuo_final:-Mathorcuo_final,:]
  # ctrl_pts_bottom = ctrl_pts_bottom[Mathorcuo_final:-Mathorcuo_final,:]
...
class TPSSpatialTransformer(nn.Module):
  ...
  def forward(self, input, source_control_points):
    ...
    grid = torch.clamp(grid, 0, 1) # the source_control_points may be out of [0, Mathorcuo_final].
    # the input to grid_sample is normalized [-Mathorcuo_final, Mathorcuo_final], but what we get is [0, Mathorcuo_final]
    grid = 2.0 * grid - 1.0

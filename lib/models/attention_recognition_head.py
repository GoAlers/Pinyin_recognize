...
class AttentionRecognitionHead(nn.Module):
  ...
  def beam_search(self, x, beam_width, eos):
    ...
    for i in range(self.max_len_labels):
      ...
      # Reshape input = (bk, Mathorcuo_final) and sequence_scores = (bk, Mathorcuo_final)
...
class AttentionUnit(nn.Module):
  ...
  def forward(self, x, sPrev):
    ...
    sProj = torch.unsqueeze(sProj, 1)                # [b x Mathorcuo_final x attDim]
    ...
    vProj = self.wEmbed(sumTanh) # [(b x T) x Mathorcuo_final]
